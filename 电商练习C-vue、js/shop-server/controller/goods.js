...
const getList = async (req, res, next) => {
  ...
  if (category && !clist.some((item) => item === category))
    return next({
...
const removeGoosByIdController = async (req, res, next) => {
  const result = await removeGoosById(req.params.goods_id);
...
const updateGoosByIdController = async (req, res, next) => {
  const result = await updateGoosById(req.params.goods_id, req.body);
