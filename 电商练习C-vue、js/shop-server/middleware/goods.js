const getList = (req, res, next) => {
  // 参数验证
  let { current, pagesize, search, filter, saleType, sortType, sortMethod } =
    req.query;
  ...
  sortMethod === undefined && (req.query.sortMethod = sortMethod = "DESC");
  ...
  if (!/^\d*$/.test(pagesize))
    return next({
      message:
        "pagesize 内容不对哦 (#^.^#), 需要的是一个数字, 但是你给的并不是",
      code: 5,
      tips: `你传递给后端的 pagesize 内容是 : '${pagesize}'`,
    });

  // 验证 filter
  if (!/^([a-z]+?,??)*?$/gi.test(filter))
    return next({
  ...
  // 验证 sortType
  if (!/^(id|price|sale)$/i.test(sortType))
    return next({
