...
router.get("/list", (req, res, next) => {
  ...
  mysql
    .paging(Product, {}, { _id: 0, __v: 0 }, count, limitNum)
    .then((data) => {
      res.send({
        code: "200",
        message: "获取商品分页列表数据",
...
router.get("/seckilllist", (req, res, next) => {
  ...
  mysql
    .paging(Product, { isseckill: 1 }, { _id: 0, __v: 0 }, count, limitNum)
    .then((data) => {
      res.send({
        code: "200",
        message: "获取秒杀产品列表的数据",
...
router.get("/recommendlist", (req, res, next) => {
  ...
  mysql
    .paging(Product, { isrecommend: 1 }, { _id: 0, __v: 0 }, count, limitNum)
    .then((data) => {
      res.send({
        code: "200",
        message: "获取推荐产品列表的数据",
...
router.get("/categorybrandprolist", (req, res, next) => {
  ...
  mysql
    .paging(Product, { category, brand }, { _id: 0, __v: 0 }, count, limitNum)
    .then((data) => {
      if (data.length === 0) {
        res.send({
          code: "10003",
...
router.get("/uploadPro", (req, res, next) => {
  ...
  mysql.delete(Product, {}, 1).then(() => {
    // 不要忘记写1，因为1 代表的是删除多条数据
    // 所有的数据已删除完成
    // 插入数据
